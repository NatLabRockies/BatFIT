...
def gather3DArray(array_, rootId, N1Loc, N1Glob, N2, N3):
    # ~~~ The parallelization is across the axis 0
    # ~~~ This will not work if the parallelization is across axis 1
    # Broadcast type
    dtype = None
    ...
    dtype = bcast(dtype)
    # Reshape the local data matrices:
    nElements_ = N1Loc * N2 * N3
    sendbuf = np.reshape(array_, nElements_, order="C")
    # Collect local array sizes:
    sendcounts = comm.gather(len(sendbuf), root=rootId - 1)
    # Gather the data matrix:
    recvbuf = np.empty(N1Glob * N2 * N3, dtype=dtype)
    comm.Gatherv(
        sendbuf=sendbuf, recvbuf=(recvbuf, sendcounts), root=rootId - 1
    )
    recvbuf = np.reshape(recvbuf, (N1Glob, N2, N3), order="C")
    return recvbuf


def gather_variable_numpy_arrays(array_, rootId):
    # Ensure array is contiguous before sending
