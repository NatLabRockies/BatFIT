...
def test_identifiability_shape():
...
def test_identifiability_known_value():
...
def test_accuracy_zero_when_equal():
    x = torch.ones(8, 4)
    result = accuracy(x, x)
...
def test_rel_accuracy_zero_when_equal():
    x = torch.ones(8, 4)
    result = rel_accuracy(x, x)
    assert torch.all(result == 0.0)


def test_rel_accuracy_shape():
...
def test_rel_accuracy_known_value():
