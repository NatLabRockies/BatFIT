...
elif sim_params["cyc_mode"].lower() in ["diffcap", "prehppc"]:
    multi_run(
        sim_params=sim_params,
        parallel_env=parallel_env,
        folder_save=args.folder_save,
        n_points_reduce=1024,
    )
elif sim_params["cyc_mode"].lower() in ["hppc"]:
    multi_run(
        sim_params=sim_params,
        parallel_env=parallel_env,
        folder_save=args.folder_save,
...
elif sim_params["cyc_mode"].lower() in ["posthppc"]:
    multi_run(
        sim_params=sim_params,
        parallel_env=parallel_env,
        folder_save=args.folder_save,
        n_points_reduce=8192,
    )
else:
    multi_run(
        sim_params=sim_params,
