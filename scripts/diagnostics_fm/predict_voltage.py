...
class ForwardModel:
    ...
    def __call__(self, degradation_parameters: list):
        deg_param_sample = {}
        for ideg_param, deg_param_name in enumerate(self.sim_params["deg_param_names"]):
...
def draw_deg_parameter_samples(deg_parameters: dict, n_samples: int = 10) -> np.ndarray:
...
def do_voltage_pred(rpt_id: int, data_type: str, dim:int=12, n_samples=10):
    ...
    voltage_output_folder = os.path.join(voltage_output_folder, data_type.lower())
    os.makedirs(voltage_output_folder, exist_ok=True)
    voltage_output_folder = os.path.join(voltage_output_folder, f"{dim}dim")
    os.makedirs(voltage_output_folder, exist_ok=True)
    param_output_folder = os.path.join("output", data_type.lower(), f"{dim}dim")
    if rpt_id == -1:
        folder_leaf = "BOL"
    else:
        folder_leaf = f"RPT_{rpt_id}"
    voltage_output_folder_leaf = os.path.join(voltage_output_folder, folder_leaf)
    ...
    with open(os.path.join(param_output_folder_leaf, "deg_parameters.pkl"), "rb") as f:
        deg_parameters = pickle.load(f)

    # Load target data
    target_folder = (
        f"/projects/mlbatt/LHX_2/extract_hdvolts_data/data_target/{folder_leaf}"
    )
    ...
    #for protocol in ["LH-1"]:
        ...
        target_file = os.path.join(target_folder, f"{protocol}_{data_type.lower()}.pkl")
    ...
    for protocol, cell_id, t_tmp, phi_tmp in results:
        ...
        if cell_id not in phi_pred[protocol]:
            phi_pred[protocol][cell_id] = {
                ...
                "phi_target": target_data_store[(protocol, cell_id)]["phi_target"],
                "error": []
            }
        ...
        if phi_tmp is not None and t_tmp is not None:
            ...
            t_interp = np.linspace(max(t_target.min(), t_tmp.min()), min(t_target.max(), t_tmp.max()), 2048)
            phi_target_interp = np.interp(t_interp, t_target, phi_target)
            phi_sim_interp = np.interp(t_interp, t_tmp, phi_tmp)
            phi_pred[protocol][cell_id]["error"].append(1000*np.mean(abs(phi_sim_interp-phi_target_interp)))
        else:
