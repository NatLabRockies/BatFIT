...
def plot_voltage(data_type, rpt_id, dim):
    ...
    for protocol in phi_pred:
        for cell_id in phi_pred[protocol]:
            ...
            mean_mae = np.nanmean(
                np.array(phi_pred[protocol][cell_id]["error"])
            )
            ...
            pretty_labels(
                "t [s]",
                r"$\phi$ [V]",
                title=f"MAE = {mean_mae:.2g} +/- {std_mae:.2g} mV;  Min-max error = {min_mae:.2g}-{max_mae:.2g} mV",
                fontsize=16,
                fontname="Times",
                grid=False,
            )
            plt.savefig(
                os.path.join(figure_folder, f"{protocol}_{cell_id}.png")
            )
            plt.close()
