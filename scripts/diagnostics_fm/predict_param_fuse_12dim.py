...
def classify_samples(candidate_samples):
    ...
    config = "/projects/mlbatt/LHX_2/BatFIT/batfit/default_exps/p2d_diffcap_12dim_ext.yaml"
    mins, maxs, param_names = load_config(config)
    model = JointBoundaryClassifier(input_dim=12, hidden_dim=256, num_layers=2)
    model = load_model(
        model,
        "/scratch/mhassana/LHX_2/train_ext/fuse_post_12dim_ext/models/model_final.pt",
    )
    device = model.device
    model.eval()
...
def get_combined_posterior(rpt_id, output_folder="output", n_samples=100000):
    output_posthppc = os.path.join(output_folder, "posthppc/12dim")
    output_diffcap = os.path.join(output_folder, "diffcap/12dim")
    if rpt_id == -1:
        folder_leaf = "BOL"
    else:
...
if __name__ == "__main__":
    ...
    for data_type in ["diffcap-posthppc"]:
        output_folder2 = os.path.join(
            output_folder, data_type.lower(), "12dim"
        )
