...
def classify_samples(candidate_samples):
    sys.path.append("/projects/mlbatt/LHX_2/fuse_post_19dim_ext")
    from classifier import JointBoundaryClassifier
    from fuse_utils import load_config

    config = "/projects/mlbatt/LHX_2/BatFIT/batfit/default_exps/p2d_diffcap_19dim_ext.yaml"
    mins, maxs, param_names = load_config(config)
    model = JointBoundaryClassifier(input_dim=19, hidden_dim=256, num_layers=2)
    model = load_model(
        model,
        "/scratch/mhassana/LHX_2/train_ext/fuse_post_19dim_ext/models/model_final.pt",
    )
    device = model.device
    model.eval()
...
def get_combined_posterior(rpt_id, output_folder="output", n_samples=100000):
    output_posthppc = os.path.join(output_folder, "posthppc/19dim")
    output_diffcap = os.path.join(output_folder, "diffcap/19dim")
    if rpt_id == -1:
        folder_leaf = "BOL"
    else:
...
if __name__ == "__main__":
    ...
    for data_type in ["diffcap-posthppc"]:
        output_folder2 = os.path.join(
            output_folder, data_type.lower(), "19dim"
        )
