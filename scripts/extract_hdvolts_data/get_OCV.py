...
def get_raw_ca_OCV(root_folder="/Users/mhassana/Desktop/GitHub/HDVOLTS_data"):
    ...
    raw = pd.read_csv(os.path.join(half_cell_data_folder,"NMC C-50 raw.csv"), skiprows=0)
    #breakpoint()
    return raw

def get_raw_an_OCV(root_folder="/Users/mhassana/Desktop/GitHub/HDVOLTS_data"):
    half_cell_data_folder = os.path.join(root_folder, "Half cell")
    raw = pd.read_csv(os.path.join(half_cell_data_folder,"Gr-Si C-50 raw.csv"), skiprows=0)
    #breakpoint()
    return raw

def get_an_OCV(root_folder="/Users/mhassana/Desktop/GitHub/HDVOLTS_data"):
    half_cell_data_folder = os.path.join(root_folder, "Half cell")
    raw = pd.read_csv(os.path.join(half_cell_data_folder,"Gr-Si OCV C-50.csv"), skiprows=0)
...
def get_ca_OCV(root_folder="/Users/mhassana/Desktop/GitHub/HDVOLTS_data"):
    half_cell_data_folder = os.path.join(root_folder, "Half cell")
    raw = pd.read_csv(os.path.join(half_cell_data_folder,"NMC OCV C-50.csv"), skiprows=0)
    SOC = raw["SOC_avg (%)"]*0.01
    OCV = raw["Voltage_avg (V)"]
    return SOC, OCV

def get_xmin_nmc811():
    array = pd.read_csv("nmc811_garrick.png.traj.csv", skiprows=0, delimiter=" ").to_numpy()
    array[:,0] = array[:,0]/10
...
if __name__ == "__main__":
    ...
    OCV_add = OCV_ca_extrap[-1] + (X_add - X_ca_extrap[-1])/(1.0 - X_ca_extrap[-1]) * (2.5 -OCV_ca_extrap[-1])
    ...
    OCV_add = OCV_an_extrap[-1] + (X_add - X_an_extrap[-1])/(1.0 - X_an_extrap[-1]) * (0.0 -OCV_an_extrap[-1])
    OCV_an_extrap = np.delete(OCV_an_extrap, [-1])
    X_an_extrap = np.delete(X_an_extrap, [-1])
