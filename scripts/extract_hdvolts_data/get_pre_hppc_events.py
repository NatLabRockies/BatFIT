    ...
    for rpt_id in file_names[protocol]:
        for cell_in in file_names[protocol][rpt_id]:
            ...
            if step_3_time is None:
                ...
                step_3_time = np.hstack(
                    (step_3_time, get_duration(list_of_dfs[2]))
                )
            if step_4_amp is None:
                step_4_amp = get_current(list_of_dfs[3])[1:]
            else:
                step_4_amp = np.hstack(
                    (step_4_amp, get_current(list_of_dfs[3])[1:])
                )
            if step_5_time is None:
                step_5_time = get_duration(list_of_dfs[4])
            else:
                step_5_time = np.hstack(
                    (step_5_time, get_duration(list_of_dfs[4]))
                )

print(
    f"STEP 1 CC Charge = {np.mean(step_1_amp):.10g}A +/- {np.std(step_1_amp):.5g}A "
)
print(
    f"STEP 3 Rest Time = {np.mean(step_3_time):.10g} min +/- {np.std(step_3_time):.5g} min "
)

print(
    f"STEP 4 CC Discharge = {np.mean(step_4_amp):.10g}A +/- {np.std(step_4_amp):.5g}A "
)
print(
    f"STEP 5 Rest Time = {np.mean(step_5_time):.10g} min +/- {np.std(step_5_time):.5g} min "
)
