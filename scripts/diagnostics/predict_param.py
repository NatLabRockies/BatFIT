...
def do_diagnostics(rpt_id:int, data_type:str):
    ...
    for protocol in pairs:
        ...
        for cell_id in target_data:
            ...
            mu, sigma = single_forward_pass(target_data_cell["t"], target_data_cell["phis_c"], scaler_file, model)
            deg_parameters[protocol][cell_id] = {"mu": np.squeeze(mu), "sigma":np.squeeze(sigma)}
