...
from batfit import BATFIT_DIR, BATFIT_EXP
...
def classify_samples(candidate_samples):
    sys.path.append("/Users/mhassana/Desktop/GitHub/BatFIT_mar26/scripts/fuse_post") 
    ...
    model = JointBoundaryClassifier(
        input_dim=31, hidden_dim=256, num_layers=2
    )
    model = load_model(model, "/Users/mhassana/Desktop/GitHub/BatFIT_mar26/scripts/fuse_post/train_log/model_final.pt")
    device = model.device
    model.eval()
...
def get_mu_sigma_combined(deg_parameters_hppc, deg_parameters_diffcap, n_samples=100000):
    deg_parameters = {}
    for protocol in deg_parameters_hppc:
        ...
        for cell_id in deg_parameters_hppc[protocol]:
            ...
            for i in range(n_params):
                sigma[i] = 1.0/hppc["sigma"][i]**2 + 1.0/diffcap["sigma"][i]**2
                sigma[i] = np.sqrt(1.0/sigma[i])
                mu[i] = sigma[i]**2 * (hppc["mu"][i]/ hppc["sigma"][i]**2 + diffcap["mu"][i]/ diffcap["sigma"][i]**2)

            samples = np.random.normal(loc=mu, scale=sigma, size=(n_samples, len(mu)))
...
def get_combined_posterior(rpt_id, output_folder="output", n_samples=100000):
    ...
    with open(os.path.join(output_hppc, folder_leaf, "deg_parameters.pkl"), "rb") as f:
        deg_parameters_hppc = pickle.load(f)
    with open(os.path.join(output_diffcap, folder_leaf, "deg_parameters.pkl"), "rb") as f:
        deg_parameters_diffcap = pickle.load(f)

    deg_parameters_combined = get_mu_sigma_combined(deg_parameters_hppc, deg_parameters_diffcap)
...
if __name__ == "__main__":
    ...
    for data_type in ["diffcap-hppc"]:
        ...
        for rpt_id in [-1, 1, 2, 3, 4, 5, 6, 7, 8]:
            ...
            deg_param = get_combined_posterior(rpt_id, output_folder=output_folder, n_samples=100000)
            with open(os.path.join(output_folder3, "deg_parameters.pkl"), "wb") as f:
