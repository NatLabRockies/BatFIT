...
parser = argparse.ArgumentParser(description="Parameter sampling (chirp mode)")
parser.add_argument(
    "-n_int",
    "--n_int",
    type=int,
...
if n_int > 0:
    deg_int_samples, prot_int_samples = get_samples(
        ...
        prot_param_names=prot_param_names,
...
if n_bound > 0 and n_int > 0:
    ...
    if prot_int_samples is not None and prot_bound_samples is not None:
        prot_samples = np.vstack((prot_int_samples, prot_bound_samples))
if n_bound == 0:
    deg_samples = deg_int_samples
    prot_samples = prot_int_samples
if n_int == 0:
    deg_samples = deg_bound_samples
    prot_samples = prot_bound_samples
